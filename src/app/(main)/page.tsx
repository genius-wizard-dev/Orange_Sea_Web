...
const Page: React.FC = () => {
	...
	const onlineUsers = useSelector((state: RootState) => state.chat.onlineUsersByGroup);
	...
	const getOnlineStatus = (userId: string) => {
		...
		const isOnline = activeGroupId && onlineUsers[activeGroupId]?.includes(userId) || false;
	...
	useEffect(() => {
		...
		socket.on("notifyMessage", (data) => {
			...
			if (type === "NEW_MESSAGE") {
				...
				dispatch(setUnreadCount(unreadCounts)); // chatSlice
				dispatch(setUnreadCountsToGroups(unreadCounts)); // groupSlice
		...
		socket.on("messageRecalled", (data) => {
			...
			dispatch(markMessagesAsRead({ groupId, messageIds: [messageId], profileId: userProfile.id }));
			dispatch(addMessage({ groupId, message: mapServerMessageToClient(recalledMessage) }));
			// Update last message in group
			if (wasLastMessage) {
				dispatch(updateLastMessage({
					groupId: groupId,
					message: recalledMessage.content,
					time: recalledMessage.createdAt,
				}));
			}
									...
									searchResults.length > 0 ? (
										<ScrollArea className="w-full">
											{searchResults.map((group) => {
												...
												const online = getOnlineStatus(group.participants?.find(p => p.id !== userProfile?.id)?.id || "");
						...
						<div className="flex flex-col">
							...
							{activeGroup?.isGroup ? (
								...
								<span className={cn(
									...
									getOnlineStatus(activeGroup?.participants?.find(p => p.id !== userProfile?.id)?.id || "") === "ACTIVE"
										? "text-green-500"
										: getOnlineStatus(activeGroup?.participants?.find(p => p.id !== userProfile?.id)?.id || "") === "ONLINE"
											? "text-yellow-500"
											: "text-gray-500"
								)}>
									{getOnlineStatus(activeGroup?.participants?.find(p => p.id !== userProfile?.id)?.id || "") === "ACTIVE"
